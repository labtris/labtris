...
async def _spine_leaf(b: _Builder, body: GenerateIn) -> None:
    ...
    for i in range(body.spines):
        n = await b.add_node(f"spine-{i+1}", body.spine_kind, row=0, col=i, row_wide=body.spines,
                             opts={"p4_program": body.p4_program} if body.spine_kind == "bmv2" else None)
        spines.append(n)

    for j in range(body.leaves):
        n = await b.add_node(f"leaf-{j+1}", body.leaf_kind, row=1, col=j, row_wide=body.leaves)
        leaves.append(n)
